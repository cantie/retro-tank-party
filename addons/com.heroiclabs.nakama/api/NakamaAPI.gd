...
class ApiClient extends RefCounted:
	...
	var auto_retry : bool:
	set = _set_retry, get = _get_retry
	var auto_retry_count : int:
	set = _set_retry_count, get = _get_retry_count
	var auto_retry_backoff_base : int:
	set = _set_retry_backoff, get = _get_retry_backoff
	...
	func update_account_async(
		...
		var content : PackedByteArray
		content = JSON.stringify(p_body.serialize()).to_utf8()
	...
	func rpc_func_async(
		...
		var content : PackedByteArray
		content = JSON.stringify(p_body).to_utf8()
